...
class CampaignShow extends Component {
  ...
  onSubmit = async (event) =>  {
    ...
    await lottery.methods.placeBid(2).send({
      from: accounts[0],
  ...
  onendauction = async () => {
    const accounts= await web3.eth.getAccounts();
    await lottery.methods.endAuction(2).send({
      from:accounts[0]
    });
  };
  ...
  static async getInitialProps(props) {
    ...
    const summary = await lottery.methods.getAuction(2).call();
  ...
  render() {
    ...
    <button>Enter</button>
     ...
     <button onClick={this.onendauction}>Beneficiary can click here and end the auction</button>
    <hr />
